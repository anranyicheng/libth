...
LAB_IMPLEMENT_BASIC_FUNCTION(gamma,TH_MATH_NAME(tgamma))
...
THTensor_(polygamma)(long n, THTensor *input, THTensor *output) {
  long nInput = THTensor_(nElement)(input);
  long nOutput = THTensor_(nElement)(output);
  if (nInput != nOutput) {
    THError("polygamma: input and output should have the same element count.");
    return;
  }

#if defined(TH_REAL_IS_DOUBLE)
  TH_TENSOR_APPLY2(real, input, real, output,
                   *output_data = polygamma_impl_dbl(n, *input_data););
#else
  TH_TENSOR_APPLY2(real, input, real, output,
                   *output_data = polygamma_impl(n, *input_data););
#endif
}

void
THTensor_(lbeta)(THTensor *a, THTensor *b, THTensor *output) {
  long nA = THTensor_(nElement)(a);
  long nB = THTensor_(nElement)(b);
  long nOutput = THTensor_(nElement)(output);
  if (nA != nB || nA != nOutput || nB != nOutput) {
    THError("lbeta: a, b and output should have the same element count.");
    return;
  }

#if defined(TH_REAL_IS_DOUBLE)
  TH_TENSOR_APPLY3(real, a, real, b, real, output,
                   *output_data = lbeta_impl_dbl(*a_data, *b_data););
#else
  TH_TENSOR_APPLY3(real, a, real, b, real, output,
                   *output_data = lbeta_impl(*a_data, *b_data););
#endif
}
